...
Client Tier (Flutter App): Handles all user interactions, UI state, and mapping logic. The application connects to Firestore directly via cloud_firestore to receive instant, real-time updates when a disruption occurs or is resolved. It also consumes Server-Sent Events (SSE) to display the live thoughts of the AI during generation.

API Tier (Express + LangGraph): Acts as the secure middleware and orchestrator.
...
Data & Retrieval Tier (Chroma & Firestore):
...
The Flutter web application is configured to deploy automatically via AWS Amplify. An amplify.yml build specification is included in the repository root.

Steps:

Go to the AWS Amplify Console.
Connect your Git repository.
Amplify will automatically detect the amplify.yml and set up continuous deployment.
Your app will be served globally via Amazon CloudFront.
Backend (Cloud Run / AWS ECS)
Because the backend now requires containerization (for Chroma DB), you must deploy the docker-compose setup to a container hosting service. Note: This replaces the previous Firebase Cloud Functions deployment, which does not natively support persistent local Vector Databases.

Push the backend/ Docker image to Amazon ECR or Google Artifact Registry.
Deploy the container to Google Cloud Run or AWS Fargate.
Provision a managed Chroma DB or Pinecone instance if you prefer not to self-host the vector store.
Update the Flutter ApiService.dart to point to your new production backend URL.
